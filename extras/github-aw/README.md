# github-aw

Track a real factory task through GitHub Actions.

## Install

Homebrew installation awaits the next release and tap update. Nix installation is available from the repository.

```sh
nix profile add 'github:roshbhatia/orc?dir=extras#provider-github-aw'
```

Install the core utility separately, or select its all-provider bundle. Runtime tools still need their own credentials.

See [usage and dispatch receipts](usage.md). The first live recording requires an authenticated factory run.

## Demo

The first live recording requires an authenticated factory run. [Tape source](demo.tape) and [task script](demo.sh) are available.
