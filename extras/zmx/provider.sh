...
zmx_process_identity() {
  local name pid created tree_helper
  name=$1
  pid=$2
  created=$3
  tree_helper=${ORC_ZMX_TREE_HELPER:-orc-provider-zmx-process-tree}
  if ! command -v "$tree_helper" >/dev/null; then
    printf 'orc-provider-zmx: process-tree helper is unavailable for %s\n' "$name" >&2
    return 2
  fi
  "$tree_helper" inspect --name "$name" --pid "$pid" --created "$created"
}

parse_zmx_ref() {
  local ref tail remainder
  ref=$1
  parsed_name=$ref
  parsed_pid=
  parsed_created=
  ...
  remainder=${ref%@*}
  if [[ $tail == *.* ]]; then
    parsed_identity=$tail
    parsed_created=${remainder##*@}
    remainder=${remainder%@*}
  else
    parsed_created=$tail
  fi
  parsed_pid=${remainder##*@}
  parsed_name=${remainder%@*}
}
...
if [[ ${1:-} == stop ]]; then
  ...
  expected_identity=${5:?precise process identity is required}
  : "${6:?Orc operation ID is required}"
...
session.bind)
  ...
  parse_zmx_ref "$existing_ref"
  existing_name=$parsed_name
  existing_pid=$parsed_pid
  existing_created=$parsed_created
  ...
  if [[ $rebind_current == true ]] && current_session_matches && [[ -n ${ZMX_SESSION:-} ]]; then
    ...
    if ((record_status == 0)); then
      IFS=$'\t' read -r current_pid current_created current_name <<<"$record"
      current_identity=$(zmx_process_identity "$current_name" "$current_pid" "$current_created")
      emit_binding "persistence" "active" "$current_name@$current_pid@$current_created@$current_identity" "Zmx session $current_name"
    else
...
session.persist)
  ...
  if [[ -n $provider_ref ]]; then
    parse_zmx_ref "$provider_ref"
    provider_name=$parsed_name
...
session.stop)
  ...
  if [[ $(jq -r '.preflight // false' <<<"$request") == true ]]; then
  ...
  if [[ -s $operation_path ]]; then
    IFS=$'\t' read -r recorded_operation provider_name provider_pid provider_created provider_identity <"$operation_path"
  fi
  if [[ ${recorded_operation:-} != "$operation_id" || -z ${provider_identity:-} ]]; then
    parse_zmx_ref "$provider_ref"
    provider_name=$parsed_name
    provider_pid=$parsed_pid
    provider_created=$parsed_created
    provider_identity=$parsed_identity
    ...
    if [[ -z $provider_identity ]]; then
      provider_identity=$(zmx_process_identity "$provider_name" "$provider_pid" "$provider_created")
    fi
    umask 077
    mkdir -p "$operation_directory"
    temporary=$operation_path.$$.tmp
    printf '%s\t%s\t%s\t%s\t%s\n' "$operation_id" "$provider_name" "$provider_pid" "$provider_created" "$provider_identity" >"$temporary"
    mv "$temporary" "$operation_path"
  fi
  provider_self=${ORC_PROVIDER_SELF:-$0}
  emit_plan "$scope" '{}' "$provider_self" stop "$provider_name" "$provider_pid" "$provider_created" "$provider_identity" "$operation_id"
  ;;
